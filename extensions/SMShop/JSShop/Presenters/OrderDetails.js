...
JSShop.Presenters.OrderDetails = function(model)
{
	...
	function init()
	{
		...
		if (document.querySelector("link[href*='/Views/OrderDetails.css']") === null) // Might have been loaded by CMS to prevent flickering (FOUC - flash of unstyled content)
			Fit.Loader.LoadStyleSheet(JSShop.GetPath() + "/Views/OrderDetails.css?CacheKey=" + (JSShop.Settings.CacheKey ? JSShop.Settings.CacheKey : "0"));

		var req = new Fit.Http.Request(JSShop.GetPath() + "/Views/OrderDetails.html?CacheKey=" + (JSShop.Settings.CacheKey ? JSShop.Settings.CacheKey : "0"));
